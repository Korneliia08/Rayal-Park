...
document.addEventListener("scroll", () => {
    ...
    blockForLogoAndBurger.classList.add("fixedNavBurger")
    if (window.scrollY === 0) {
        nav.classList.toggle("fixedNav");
        blockForLogoAndBurger.classList.toggle("fixedNavBurger")
...
closeButton.addEventListener('click', () => {
    navBurger.style.top = '-200px'
})
Array.from(aHrefInNav).forEach(element => {
    console.log(2);
